defmodule Microblog.Blog do
  ...
  def already_liked(user_id, post_id) do
    Repo.all(from l in Like, where: l.post_id == ^post_id and l.user_id == ^user_id)
    |> Repo.preload(:post)
    |> Repo.preload(:user)
  end

  def already_follow(current_user_email, user_email) do
    Repo.all(from f in Follower, where: f.email_me == ^current_user_email and f.email_owner == ^user_email)
  end


  def show_like_count(post_id) do
